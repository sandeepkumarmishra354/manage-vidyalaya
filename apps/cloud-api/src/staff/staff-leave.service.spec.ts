...
import type { LeaveTypesService } from "../leave-types/leave-types.service.js";
...
function makeLeaveTypesMock() {
  return {
    resolveMonthlyAccrual: vi.fn(),
    usedDaysInYear: vi.fn(),
    getBalance: vi.fn(),
  } as unknown as LeaveTypesService;
}

const TYPE_LOOKUP_ROW = { rows: [{ id: "type-1", tenant_id: "tenant-1", quota_enabled: false }] };
...
describe("StaffLeaveService.apply", () => {
  ...
  it("creates a pending request for the caller's own linked staff row", async () => {
    ...
    client.query
      .mockResolvedValueOnce(TYPE_LOOKUP_ROW)
      .mockResolvedValueOnce({ rows: [{ id: "req-1", status: "pending", tenant_id: "tenant-1" }] });

    await service.apply("tenant-1", "user-1", {
  ...
  it("rejects when the logged-in user has no linked staff record", async () => {
    ...
    await expect(
      service.apply("tenant-1", "user-1", { leave_type_id: "type-1", start_date: "2026-04-10", end_date: "2026-04-12" }),
  ...
  it("rejects an end date before the start date", async () => {
    ...
    await expect(
      service.apply("tenant-1", "user-1", { leave_type_id: "type-1", start_date: "2026-04-12", end_date: "2026-04-10" }),
    ).rejects.toThrow();
  });
  ...
  it("allows an on_leave staff member to apply", async () => {
    ...
    await service.apply("tenant-1", "user-1", { leave_type_id: "type-1", start_date: "2026-04-10", end_date: "2026-04-12" });
...
describe("StaffLeaveService.cancel", () => {
  ...
  beforeEach(() => {
    ...
    audit = makeAuditMock();
    scopedAccess = makeScopedAccessMock();
    service = new StaffLeaveService(db, audit, scopedAccess, makeLeaveTypesMock());
  });
...
describe("StaffLeaveService.file (HR on-behalf)", () => {
  ...
  it("creates the request already approved and writes attendance for every date in range", async () => {
    client.query.mockImplementation(async (text: unknown) => {
      ...
      if (typeof text === "string" && text.includes("FROM leave_types")) {
        return TYPE_LOOKUP_ROW;
      }
...
describe("StaffLeaveService.decide", () => {
  ...
  it("splits a request into paid/unpaid days once a quota-enabled leave type's balance runs out", async () => {
    client.query.mockImplementation(async (text: unknown) => {
      if (typeof text === "string" && text.includes("FROM staff_leave_requests lr")) {
        return {
          rows: [
            {
              ...
              branch_id: "branch-1",
              staff_id: "staff-1",
              leave_type_id: "type-casual",
              start_date: new Date("2026-04-10T00:00:00.000Z"),
              end_date: new Date("2026-04-14T00:00:00.000Z"),
              is_half_day: false,
              staff_first_name: "Asha",
              staff_last_name: "Rao",
              staff_category_id: "cat-1",
              staff_date_of_joining: new Date("2020-01-01T00:00:00.000Z"),
            },
          ],
        };
      }
      if (typeof text === "string" && text.includes("FROM leave_types")) {
        return { rows: [{ id: "type-casual", tenant_id: "tenant-1", name: "Casual Leave", quota_enabled: true }] };
      }
      if (typeof text === "string" && text.startsWith("UPDATE staff_leave_requests")) {
        return { rows: [{ id: "req-1", status: "approved", tenant_id: "tenant-1" }] };
      }
    ...
    // 1 day/month accrued, nothing used yet this year -> 4 days accrued by April.
    ...
    expect(attendanceCalls).toHaveLength(5); // 10-14 April inclusive
    const paidCalls = attendanceCalls.filter(([, params]) => (params as unknown[]).includes("leave"));
    const unpaidCalls = attendanceCalls.filter(([, params]) => (params as unknown[]).includes("leave_unpaid"));
    expect(paidCalls).toHaveLength(4);
    expect(unpaidCalls).toHaveLength(1);
